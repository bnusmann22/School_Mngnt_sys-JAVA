
# 👥 Contributors

This project was developed as a **collaborative Java OOP group project**.  
Each contributor worked on **specific modules** of the School Management System.

---

## 🧑‍💻 Project Lead / Coordinator
- **Abdullahi Jamil Muhammad**  
  *Role:* Project coordination, system integration, core structure  
  *GitHub:* https://github.com/bnusmann22

---

## 📦 Module Contributors

### 🎓 Student Management Module
- M. J Abdullahi (CST/23/SWE/00989) – Student data model & logic
- Suleiman Abba Abubakar (CST/23/SWE/00981 ) – Student listing & display
- Abubakar Saleh Muhammad (CST/23/COM/01093) – Input validation (if applicable)

---

### 👩‍🏫 Teacher Management Module
-  MUHAMMAD ANSAR HAMZA (CST/23/IFT/00927) – Teacher class & attributes
- Mardiyyah Muhammad Mukhtar (CST/24/SWE/00201) – Teacher registration logic

---

### 📚 Course Management Module
- Umar Shuaibu AbdulKarim (CST/23/SWE/) – Course creation & storage
- Abdulsalam Danjuma Suleiman (CST/23/IFT/00995) – Course listing & assignment

---

### 📅 Attendance Module
- Basheer Olanrewaju Abdul-Salam (CST/23/SWE/00985 ) – Attendance tracking
- DAVID JEDIDIAH AKHALU (CST/23/IFT/01003) – Attendance reporting

---

### 📝 Result & Grading Module
- Ibrahim Ibrahim Ahmad (CST/23/SWE/01029) – Result model
- Aaron Akachukwu (CST/23/SWE/01001) – Grade calculation logic

---

### 💰 Fee Management Module
- Contributor 1 – Fee records
- Contributor 2 – Balance & payment logic

---

## 🧠 Core System & Integration
- Menu navigation
- Module integration
- Application flow control

---

## 🤝 Contribution Policy
- Each contributor worked **only on their assigned module**
- All changes were reviewed before merging
- GitHub branches and pull requests were used for collaboration

---

## 📌 Acknowledgement
This project was created strictly for **educational purposes**  
to practice **Java programming, Object-Oriented Design, and teamwork**.
